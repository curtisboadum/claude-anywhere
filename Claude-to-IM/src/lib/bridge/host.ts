/**
 * @file host.ts
 * @description Host interfaces decoupling the bridge from any specific host app.
 *   Permission links carry the requesting userId + createdAt to support same-user
 *   binding and callback expiry.
 * @status Modified (harden/worktree-isolation): PermissionLink userId/createdAt.
 * @issues none known.
 * @todo none.
 *
 * A host must provide implementations of these interfaces to use the bridge.
 */
...
export interface PermissionLinkInput {
  ...
  toolName: string;
  suggestions: string;
  /** ID of the user who triggered the request — used for same-user callback binding. */
  userId?: string;
}
...
export interface PermissionLinkRecord {
  ...
  resolved: boolean;
  suggestions: string;
  /** ID of the user who triggered the request (same-user binding). */
  userId?: string;
  /** ISO timestamp the link was created (callback expiry). */
  createdAt?: string;
}
