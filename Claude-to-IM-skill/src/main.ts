/**
 * @file main.ts
 * @description Daemon entry point: assembles DI implementations and starts the
 *   bridge. Hardens startup (umask, permission migration, worktree GC) and
 *   refuses unsafe auto-approve mode combinations.
 * @status Modified (harden/worktree-isolation): startup hardening + session prompt.
 * @issues none known.
 * @todo none.
 *
...
import { applyPrivateUmask, hardenStateTree } from './secure-fs.js';
import { startupGc } from './worktree-manager.js';

/**
 * Refuse to start in an unsafe auto-approve configuration. Auto-approving tool
 * use without interactive confirmation is only acceptable in plan mode (where
 * tools are largely read-only) unless the operator explicitly acknowledged the
 * risk via CTI_AUTO_APPROVE_ACK=i-understand.
 */
function assertSafeAutoApprove(config: Config): void {
  if (!config.autoApprove) return;
  const safeMode = config.defaultMode === 'plan';
  if (safeMode || config.autoApproveAck) {
    console.warn('[claude-to-im] WARNING: auto-approve is ON — tool use will not be confirmed.');
    return;
  }
  console.error(
    '[claude-to-im] FATAL: CTI_AUTO_APPROVE=true is unsafe outside plan mode.\n' +
    `  Current mode: ${config.defaultMode}. Remote messages could trigger unconfirmed tool execution.\n` +
    '  Fix: set CTI_DEFAULT_MODE=plan, or remove CTI_AUTO_APPROVE, or — only if you accept the risk —\n' +
    '       set CTI_AUTO_APPROVE_ACK=i-understand.',
  );
  process.exit(1);
}
  ...
  if (runtime === 'auto') {
    ...
    if (cliPath) {
      ...
      if (check.ok) {
        ...
        return new SDKLLMProvider(pendingPerms, cliPath, config.autoApprove, config.sessionInstructions, config.autoApproveAck);
      }
...
async function main(): Promise<void> {
  // ── Startup hardening (before any state is written) ──
  applyPrivateUmask();
  const tightened = hardenStateTree(CTI_HOME);
  if (tightened > 0) {
    console.log(`[claude-to-im] Tightened permissions on ${tightened} pre-existing state path(s)`);
  }

  const config = loadConfig();
  setupLogger();
  assertSafeAutoApprove(config);

  // Reclaim stale/over-quota worktrees from previous runs.
  try { startupGc(); } catch (e) { console.warn('[claude-to-im] worktree GC failed:', e instanceof Error ? e.message : e); }
