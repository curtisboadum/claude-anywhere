/**
 * @file weixin-adapter.ts
 * @description WeChat (Weixin) channel adapter. DORMANT/fail-closed by default:
 *   processes a message only if the sender is explicitly allowlisted via
 *   CTI_WEIXIN_ALLOWED_USERS. Telegram is the supported path; WeChat lacks
 *   inbound signature verification (see SECURITY-NOTES).
 * @status Modified (harden/worktree-isolation): fail-closed auth, enforced in loop.
 * @issues No cryptographic message-signature verification (upstream protocol gap).
 * @todo Re-enable full WeChat hardening (token TTL, audit redaction) if it goes live.
 */
...
export class WeixinAdapter extends BaseChannelAdapter {
  ...
  isAuthorized(userId: string, _chatId: string): boolean {
    // Fail-closed: the WeChat adapter stays inert unless the operator explicitly
    // allowlists sender IDs via CTI_WEIXIN_ALLOWED_USERS. WeChat messages carry no
    // cryptographic signature, so without an allowlist any sender could otherwise
    // drive code execution. Telegram is the supported, verified channel.
    const raw = getBridgeContext().store.getSetting('bridge_weixin_allowed_users') || '';
  ...
  private async processMessage(accountId: string, message: WeixinMessage, batchId?: number): Promise<void> {
    ...
    // Enforce sender allowlist before any processing (fail-closed). Unauthorized
    // senders are dropped — never dispatched to the LLM.
    if (!this.isAuthorized(message.from_user_id, accountId)) {
      console.warn('[weixin-adapter] Dropping message from unauthorized WeChat sender:', message.from_user_id.slice(0, 12));
      return;
    }
