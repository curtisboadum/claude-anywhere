/**
 * @file codex-provider.test.ts
 * @description Tests Codex provider event mapping and input building. Worktree
 *   isolation is disabled here so streamChat unit tests don't spawn real
 *   worktrees; isolation itself is covered by worktree-manager.test.ts.
 * @status Modified (harden/worktree-isolation): disable isolation for unit tests.
 * @issues none known.
 * @todo none.
 */
...
// Unit tests exercise event/input mapping, not worktree isolation. Disable
// isolation so streamChat() does not create real git worktrees of this repo.
process.env.CTI_WORKTREE_ISOLATION = 'false';
