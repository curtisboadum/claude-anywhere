 ...
 * @description Claude provider via @anthropic-ai/claude-agent-sdk query(). Converts
 *   SDK stream events to the bridge SSE format, enforces per-session worktree
 *   isolation (sets cwd + appends a reinforcement system prompt), and surfaces
 *   auth/transport errors to the IM user.
 * @status Modified (harden/worktree-isolation): worktree cwd + system-prompt append.
 * @issues none known.
 * @todo none.
 */
...
import type { SDKMessage, PermissionResult, Options } from '@anthropic-ai/claude-agent-sdk';
import type { LLMProvider, StreamChatParams, FileAttachment } from 'claude-to-im/src/lib/bridge/host.js';
import type { PendingPermissions } from './permission-gateway.js';
import { ensureWorktree, renderSessionInstructions } from './worktree-manager.js';
import { concurrencyGate, rateLimiter, sessionTimeoutMs } from './abuse-guard.js';
...
/**
 * Build the Claude Agent SDK `systemPrompt` option that keeps Claude Code's
 * default preset and appends our reinforcement text. Typed against the SDK's
 * own `Options['systemPrompt']` so a future SDK shape change fails compilation
 * here (contract check), not silently at runtime.
 */
export function buildAppendedSystemPrompt(append: string): NonNullable<Options['systemPrompt']> {
...
export class SDKLLMProvider implements LLMProvider {
  ...
  private autoApproveAck: boolean;
  private sessionInstructions: string | undefined;
  ...
  constructor(
    ...
    autoApproveAck = false,
  ) {
    this.cliPath = cliPath;
    this.autoApprove = autoApprove;
    this.autoApproveAck = autoApproveAck;
  ...
  streamChat(params: StreamChatParams): ReadableStream<string> {
    ...
    const autoApproveAck = this.autoApproveAck;
    const sessionInstructions = this.sessionInstructions;
    ...
    return new ReadableStream({
      start(controller) {
        (async () => {
          ...
          // ── Abuse controls: per-session rate limit + global concurrency cap ──
          if (!rateLimiter.allow(params.sessionId)) {
            controller.enqueue(sseEvent('error', 'Rate limit exceeded — please wait a moment before sending more messages.'));
            controller.close();
            return;
          }
          if (!concurrencyGate.tryAcquire()) {
            controller.enqueue(sseEvent('error', 'The bridge is busy (max concurrent sessions reached). Try again shortly.'));
            controller.close();
            return;
          }
          ...
          try {
            ...
            // own git worktree so concurrent sessions never collide on the working
            // tree of the main checkout. Fail closed (no edits) if it cannot.
            const wt = await ensureWorktree(params.sessionId, params.workingDirectory);
            if (!wt.ok) {
              controller.enqueue(sseEvent('error', wt.error));
              controller.close();
              return;
            }
            ...
            const queryOptions: Record<string, unknown> = {
              ...
              // Reinforce (does not enforce) the isolation already applied via cwd.
              systemPrompt: buildAppendedSystemPrompt(renderSessionInstructions(wt, sessionInstructions)),
              env: cleanEnv,
              stderr: (data: string) => {
                stderrBuf += data;
              ...
              canUseTool: async (
                  ...
                  // Auto-approve in plan/code modes; ASK mode ('default') always
                  // prompts. Code-mode auto requires explicit ack (CTI_AUTO_APPROVE_ACK).
                  // This makes /mode a live toggle from the chat: /mode code → auto,
                  // /mode ask → confirm each, /mode plan → planning.
                  const autoOk =
                    autoApprove &&
                    params.permissionMode !== 'default' &&
                    (params.permissionMode === 'plan' || autoApproveAck);
                  if (autoOk) {
...
type Ctrl = ReadableStreamDefaultController<string>;

function handleStreamEvent(msg: Extract<SDKMessage, { type: 'stream_event' }>, controller: Ctrl, state: StreamState): void {
  ...
  if (event.type === 'content_block_start' && event.content_block.type === 'tool_use') {
    controller.enqueue(
      sseEvent('tool_use', { id: event.content_block.id, name: event.content_block.name, input: {} }),
    );
  }
}

function handleAssistant(msg: Extract<SDKMessage, { type: 'assistant' }>, controller: Ctrl, state: StreamState): void {
  // Full assistant message — capture text but do NOT emit it (deltas already
  // streamed). Captured text lets the catch block surface business errors that
  // arrived as assistant text without prior streaming deltas.
  if (!msg.message?.content) return;
  for (const block of msg.message.content) {
    if (block.type === 'text' && block.text) {
      state.lastAssistantText += (state.lastAssistantText ? '\n' : '') + block.text;
    } else if (block.type === 'tool_use') {
      controller.enqueue(sseEvent('tool_use', { id: block.id, name: block.name, input: block.input }));
    }
  }
}

function handleUserToolResults(msg: Extract<SDKMessage, { type: 'user' }>, controller: Ctrl): void {
  ...
  for (const block of content) {
    if (typeof block === 'object' && block !== null && 'type' in block && block.type === 'tool_result') {
      ...
      const text = typeof rb.content === 'string' ? rb.content : JSON.stringify(rb.content ?? '');
      controller.enqueue(sseEvent('tool_result', { tool_use_id: rb.tool_use_id, content: text, is_error: rb.is_error || false }));
    }
  }
}

/** Human-readable token count: 1234 -> "1.2k". */
...
/**
 * Build a compact one-line context footer from the turn's usage, e.g.
 * "— context ~48.3k / 200k (24%) · out 1.2k · /new to reset". Returns '' when
 * disabled (CTI_CONTEXT_FOOTER=false) or usage is unavailable.
 */
function contextFooter(inputTokens: number, outputTokens: number, cacheRead: number, cacheCreate: number): string {
  if (process.env.CTI_CONTEXT_FOOTER === 'false') return '';
  const ctx = (inputTokens || 0) + (cacheRead || 0) + (cacheCreate || 0);
  if (ctx <= 0) return '';
  const windowTokens = Number(process.env.CTI_CONTEXT_WINDOW) || 200_000;
  const pct = Math.round((ctx / windowTokens) * 100);
  const hint = pct >= 70 ? ' · ⚠ running low — /new to reset' : '';
  return `\n\n— context ~${fmtTokens(ctx)} / ${fmtTokens(windowTokens)} (${pct}%) · out ${fmtTokens(outputTokens || 0)}${hint}`;
}

function handleResult(msg: Extract<SDKMessage, { type: 'result' }>, controller: Ctrl, state: StreamState): void {
  state.hasReceivedResult = true;
  if (msg.subtype === 'success') {
    // Append a compact context-usage footer to the reply (before finalizing)
    // so you can see how full the context is and decide when to /new.
    const footer = contextFooter(
      msg.usage.input_tokens,
      msg.usage.output_tokens,
      msg.usage.cache_read_input_tokens ?? 0,
      msg.usage.cache_creation_input_tokens ?? 0,
    );
    if (footer) controller.enqueue(sseEvent('text', footer));
    controller.enqueue(
      sseEvent('result', {
    ...
    const errors = 'errors' in msg && Array.isArray(msg.errors) ? msg.errors.join('; ') : 'Unknown error';
...
export function handleMessage(
  ...
  switch (msg.type) {
    case 'stream_event': handleStreamEvent(msg, controller, state); break;
    case 'assistant': handleAssistant(msg, controller, state); break;
    case 'user': handleUserToolResults(msg, controller); break;
    case 'result': handleResult(msg, controller, state); break;
    case 'system':
      if (msg.subtype === 'init') {
        controller.enqueue(sseEvent('status', { session_id: msg.session_id, model: msg.model }));
      }
      break;
    default:
