/**
 * @file weixin-store.ts
 * @description Persists WeChat account + per-peer context tokens to JSON. Files
 *   contain bot/context tokens, so they are written owner-only (0600/0700).
 * @status Modified (harden/worktree-isolation): secure file/dir modes.
 * @issues WeChat adapter is disabled fail-closed by default (see weixin-adapter.ts).
 * @todo none.
 */
