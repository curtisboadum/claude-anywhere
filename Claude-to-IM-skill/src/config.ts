/**
 * @file config.ts
 * @description Loads/saves ~/.claude-to-im/config.env and maps it to upstream
 *   bridge settings. Adds worktree-isolation, abuse-control, and WeChat-allowlist
 *   knobs on top of the original channel config.
 * @status Modified (harden/worktree-isolation): new knobs + secure file modes.
 * @issues none known.
 * @todo none.
 */
...
export interface Config {
  ...
  autoApprove?: boolean;
  // Explicit acknowledgement required to allow auto-approve outside plan mode
  autoApproveAck?: boolean;
  // Custom reinforcement system prompt appended to every session ({{path}}/{{branch}} interpolated)
...
export function loadConfig(): Config {
  ...
  return {
    ...
    autoApproveAck: env.get("CTI_AUTO_APPROVE_ACK") === "i-understand",
    sessionInstructions: env.get("CTI_SESSION_INSTRUCTIONS") || undefined,
    maxConcurrent: env.get("CTI_MAX_CONCURRENT") ? Number(env.get("CTI_MAX_CONCURRENT")) : undefined,
    ratePerMin: env.get("CTI_RATE_PER_MIN") ? Number(env.get("CTI_RATE_PER_MIN")) : undefined,
    sessionTimeoutSec: env.get("CTI_SESSION_TIMEOUT_SEC") ? Number(env.get("CTI_SESSION_TIMEOUT_SEC")) : undefined,
  };
}
...
export function saveConfig(config: Config): void {
  ...
  if (config.weixinAllowedUsers)
    out += formatEnvLine("CTI_WEIXIN_ALLOWED_USERS", config.weixinAllowedUsers.join(","));
  if (config.sessionInstructions)
    out += formatEnvLine("CTI_SESSION_INSTRUCTIONS", config.sessionInstructions);
  if (config.maxConcurrent !== undefined)
    out += formatEnvLine("CTI_MAX_CONCURRENT", String(config.maxConcurrent));
  if (config.ratePerMin !== undefined)
    out += formatEnvLine("CTI_RATE_PER_MIN", String(config.ratePerMin));
  if (config.sessionTimeoutSec !== undefined)
    out += formatEnvLine("CTI_SESSION_TIMEOUT_SEC", String(config.sessionTimeoutSec));

  fs.mkdirSync(CTI_HOME, { recursive: true, mode: 0o700 });
