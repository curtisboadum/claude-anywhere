/**
 * @file codex-provider.ts
 * @description Codex (@openai/codex-sdk) LLMProvider. Maps Codex thread events to
 *   the bridge SSE format and enforces per-session worktree isolation (runs the
 *   thread inside its own git worktree; prepends a reinforcement preamble on new
 *   threads since Codex has no append-system-prompt option).
 * @status Modified (harden/worktree-isolation): worktree workingDirectory + preamble.
 * @issues Codex reinforcement is best-effort (new-thread input preamble only).
 * @todo none.
 *
      ...
      start(controller) {
        (async () => {
          ...
          try {
            ...
            // Enforce per-session worktree isolation — run the Codex thread inside
            // its own git worktree. Fail closed (no edits) if it cannot be created.
            ...
            const threadOptions: Record<string, unknown> = {
              ...
              ...(wt.cwd ? { workingDirectory: wt.cwd } : {}),
            ...
            // On a NEW isolated thread, prepend the worktree reinforcement preamble
            // (Codex has no append-system-prompt option). Resumed threads already
            // carry it; non-isolated runs leave the prompt untouched.
            const preamble = wt.isolated && !savedThreadId ? renderSessionInstructions(wt) + '\n\n' : '';
            const promptText = preamble + params.prompt;
